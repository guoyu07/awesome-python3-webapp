...
class ModelMetaclass(type):
	...
	def _new_(cls, name, bases, attrs):
		...
		for k in mappings.keys():
			attrs.pop(k)
		escaped_fields = list(map(lambda f: '`%s`' %f, fileds))  # '`%s`'what?
		attrs['_mappings_'] = mappings
		attrs['_table_'] = tableName
		attrs['_primary_key_'] = primaryKey
		attrs['_fields_'] = fileds
		attrs['_select_'] = 'select `%s`, %s from `%s`' %(primaryKey, ','.join(escaped_fields), tableName)
		...
		attrs['_update_'] = 'update `%s` set %s where `%s` = ?'
...
class Field(object):
	...
	def _str_(self):
		return '<%s, %s, %s>' % (self._class_.__name__, self.column_type, self.name)

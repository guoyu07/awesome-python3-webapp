		...
		if self._has_var_kw_arg or self._has_named_kw_args or self._required_kw_args:
			if request.method == 'POST':
				...
				if ct.startswith('application/json'):
					...
					kw = params
				elif ct.startswith('application/x-www-urlencoded') or ct.startswith('multipart/form-data'):
...
def add_route(app, fn):
	...
	if path is None or method is None:
		raise ValueError('@get or @post nor define in %s.' % str(fn))
...
def add_routes(app, modele_name):
	...
	if n == (-1):
		...
		mod = getattr(_import_(modele_name[:n], globals(), locals(),[name]),name)
	for attr in dir(mod):
